...
# ====== PAYMENT LINK (Auto-select mode) ======
def make_payment_link(*, user_id: int, amount_rub: int | float, description: str) -> tuple[str, int]:
    """
    Если в feature_flags.robokassa_keys есть поле 'subscription_url' — используем его напрямую (новая логика).
    Если нет — строим ссылку вручную (старая логика с Receipt и подписью).
    """
    rk = get_rk_settings()
