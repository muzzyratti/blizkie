...
def _paywall_text(settings: dict) -> str:
    ...
    return (
        "🧩 *Идея под момент. Всегда и без ожидания.*\n\n"
        "Вы дошли до лимита бесплатной версии.\n\n"
        "Откройте полный доступ — и бот станет вашим быстрым помощником, который "
        "в любую секунду подскажет *как провести время с ребёнком прямо сейчас*, под ваше "
