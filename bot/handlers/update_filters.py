...
async def show_update_options(event: types.Message | types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Поменять возраст", callback_data="update_age")],
            [InlineKeyboardButton(text="Поменять время", callback_data="update_time")],
            [InlineKeyboardButton(text="Поменять энергию", callback_data="update_energy")],
            [InlineKeyboardButton(text="Поменять место", callback_data="update_place")]
        ]
    )
...
async def update_age(callback: types.CallbackQuery):
    ...
    await callback.message.answer("Выберите новый возраст:", reply_markup=age_keyboard)
    await callback.answer()

@update_filters_router.callback_query(F.data == "update_time")
async def update_time(callback: types.CallbackQuery):
    user_data[callback.from_user.id]["mode"] = "update"
    await callback.message.answer("Сколько у вас есть времени на игру?", reply_markup=time_keyboard)
    await callback.answer()

@update_filters_router.callback_query(F.data == "update_energy")
async def update_energy(callback: types.CallbackQuery):
    user_data[callback.from_user.id]["mode"] = "update"
    await callback.message.answer("Сколько у вас энергии на игру?", reply_markup=energy_keyboard)
    await callback.answer()

@update_filters_router.callback_query(F.data == "update_place")
async def update_place(callback: types.CallbackQuery):
    user_data[callback.from_user.id]["mode"] = "update"
    await callback.message.answer("Где будете играть?", reply_markup=place_keyboard)
    await callback.answer()
